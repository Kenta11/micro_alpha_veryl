set project_directory "vivado/basys-3"
set project_name      "basys-3"

create_project ${project_name} ${project_directory} -part xc7a35tcpg236-1
...
set_property -dict [list CONFIG.Component_Name {main_memory} CONFIG.Write_Depth_A {1024} CONFIG.Enable_A {Always_Enabled} CONFIG.Load_Init_File {true} CONFIG.Coe_File [file normalize "${project_root}/../../fpga/basys-3/machine_program.coe"]] [get_ips main_memory]
create_ip -name blk_mem_gen -vendor xilinx.com -library ip -version 8.4 -module_name control_memory
set_property -dict [list CONFIG.Component_Name {control_memory} CONFIG.Memory_Type {Single_Port_ROM} CONFIG.Write_Width_A {40} CONFIG.Write_Depth_A {512} CONFIG.Read_Width_A {40} CONFIG.Enable_A {Always_Enabled} CONFIG.Write_Width_B {40} CONFIG.Read_Width_B {40} CONFIG.Load_Init_File {true} CONFIG.Coe_File [file normalize "${project_root}/../../fpga/basys-3/control_program.coe"] CONFIG.Port_A_Write_Rate {0}] [get_ips control_memory]
...
add_files -fileset constrs_1 "${project_root}/../../fpga/basys-3/constr.xdc"
